...
Note on terminology for this repo (Quad SFP28 FMC, MRMAC): the "lanes" field of each
design holds the list of SFP28 *ports* that the design instantiates (e.g. ["0","1","2","3"]
for all four ports). The four ports are the four clients of a single Versal MRMAC hard
block, one GTY lane per port.
'''
    ...
    for linkspeed in get_linkspeeds(data):
        ...
        for design in data['designs']:
            ...
            if not design['petalinux']:
                continue
            # Derive the BSP-overlay name from the populated ports and the
            # line rate: bsp/ports-versal-0123/ for the 10G targets and
            # bsp/ports-versal-0123-25g/ for the 25G targets. The overlays
...
def get_ignore_paths(data):
    ...
    for design in data['designs']:
        ...
        paths.append(p)
        p = 'Vitis/{}_workspace/'.format(design['label'])
        paths.append(p)
    return(paths)
...
if os.path.isfile(vitis_makefile) and args:
